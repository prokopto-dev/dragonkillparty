...
| 12 | Tier-aware auction resolution | **not started** | It did NOT land with the bid strategies: [#195](https://github.com/prokopto-dev/dragonkillparty/issues/195) shipped the four rules with the amount comparison that runs *inside* a tier, and deferred the ladder itself to [#224](https://github.com/prokopto-dev/dragonkillparty/issues/224) because tier is derived server-side at bid time and `strategy.Bid` carries no such field — so it is a shared-type change rather than arithmetic added to five planners. **The only Phase 1 deliverable still outstanding** |
