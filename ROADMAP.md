...
| 7 | Thirteen strategies | **9 of 13** | `fixed_price`, `tick`, `start_points`, `cap`, `loot_council`, `decay_percent`, `decay_window`, `zero_sum`, `attendance_weighted` — three from [#193](https://github.com/prokopto-dev/dragonkillparty/issues/193), which also added `internal/strategy/catalogue.go` (the registry a pool's rule ids resolve through, since those columns deliberately carry no CHECK) and `common.go` (the strict config decode, the zero-sum assertion and the shared reversal, one copy for every strategy). Each entry also declares which of a pool's three questions it answers — [ADR-0026](docs/adr/0026-three-rules-per-pool.md), item 14. `loot_council` ([#197](https://github.com/prokopto-dev/dragonkillparty/issues/197)) is the first whose charge is *recorded* rather than computed, and the first to refuse `Priority` on purpose; its deliberation record needs the loot tables — [#219](https://github.com/prokopto-dev/dragonkillparty/issues/219). The decay pair ([#194](https://github.com/prokopto-dev/dragonkillparty/issues/194)) widened the `Ctx` façade with `EarnedBetween` — a window expires the earnings in a slice of the log, and a balance delta over the same slice nets in the run's own prior batches — and corrected `NonNegative` to constrain a deduction rather than a balance, without which `toward_zero` debt forgiveness could plan a run and never commit one. `zero_sum` and `attendance_weighted` ([#196](https://github.com/prokopto-dev/dragonkillparty/issues/196)) are the two whose every batch is a largest-remainder split through the shared allocator, proved end to end against a real commit in `internal/ledger/pool_composition_test.go`. Remaining: [#195](https://github.com/prokopto-dev/dragonkillparty/issues/195) (the four bid strategies). `decay_window` ships the hard cutoff; the linear taper is [#221](https://github.com/prokopto-dev/dragonkillparty/issues/221), and `attendance_weighted`'s ranking score is [#223](https://github.com/prokopto-dev/dragonkillparty/issues/223) |
