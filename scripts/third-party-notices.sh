...
# with the binary — the same graph the licence gate (internal/licence) polices for forbidden
...
#
# The enumeration itself is NOT here. `licence modules` (internal/licence) resolves it, and the
# licence gate reads the same graph through the same function — the platform list used to be written
# out twice, in two dialects, under a comment asking the reader to keep them in step (issue #130).
#
...
# The union of runtime modules across every release platform, as "path\tversion\tdir" lines.
#
# The platform list, the `go list` invocation and the main-module filter all live in
# internal/licence, so this generator and the licence gate cannot drift apart about which platforms
# ship or which modules count. Diagnostics go to stderr, so nothing but module rows reaches the file;
# `set -e` stops here if the graph could not be resolved.
mods_file="$(mktemp)"
...
# `go list -deps` cannot see a font, and neither can the licence gate, which reads the same graph.
# The obligation is identical all the same — Inter's SIL OFL 1.1 requires its copyright notice and
# licence text to travel with the font, and the font travels inside the binary this file is attached
# to.
#
