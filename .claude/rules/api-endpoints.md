...
`internal/ledger/kinds` — `make gen` writes it into the CHECK and `Commit.validate` reads it
(canonical §5) — so the first DTO to expose either field derives its schema from `kinds.BatchKinds()`
/ `kinds.BatchSources()` through `huma.SchemaProvider`, exactly as `Code` does. Never an `enum`
