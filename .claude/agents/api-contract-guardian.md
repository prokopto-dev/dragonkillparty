...
| 5 | **`x-dkp-scopes` non-empty whenever `Security` offers a `pat` alternative** | A scope was invented outside `internal/authz/catalogue.go`, or an `admin:*`-shaped scope appears. There is no all-powerful token. Three cases, and they are distinct: an operation whose `Security` lists `pat` must carry non-empty `x-dkp-scopes`; an operation in canonical §6's capability floor (`token.mint`, `token.revoke`, `admin.security.manage`, `admin.roles.manage`, `admin.backup`, `admin.owner`, `person.pii.read`, `audit.read`, `import.commit`) is **session + step-up**, carries no scope, and must be marked `x-dkp-pat-forbidden: true`; an operation that is session-only merely because no scope family covers it (`admin.settings` — guild settings, servers, pools, custom fields) declares neither, and marking it PAT-forbidden is a **false positive**. Derive the middle set from canonical §6, never from a copy. |
