-- +goose Up
-- add column "strategy_config_json" to table: "pool"
ALTER TABLE "pool" ADD COLUMN "strategy_config_json" text NOT NULL DEFAULT '{}';
...
CREATE TABLE "decay_run" ("id" text NOT NULL, "pool_id" text NOT NULL, "kind" text NOT NULL, "cadence_period" text NOT NULL, "scheduled_for_at" integer NOT NULL, "executed_at" integer NULL, "state" text NOT NULL DEFAULT 'planned', "dry_run_result_json" text NOT NULL DEFAULT '{}', "config_snapshot_json" text NOT NULL DEFAULT '{}', "ledger_batch_id" text NULL, "triggered_by" text NULL, "error" text NOT NULL DEFAULT '', "created_at" integer NOT NULL, "updated_at" integer NOT NULL, PRIMARY KEY ("id"), CONSTRAINT "decay_run_pool" FOREIGN KEY ("pool_id") REFERENCES "pool" ("id") ON UPDATE NO ACTION ON DELETE NO ACTION, CONSTRAINT "decay_run_batch" FOREIGN KEY ("ledger_batch_id") REFERENCES "ledger_batch" ("id") ON UPDATE NO ACTION ON DELETE NO ACTION, CONSTRAINT "decay_run_kind_enum" CHECK (kind IN ('decay', 'cap', 'start_points')), CONSTRAINT "decay_run_state_enum" CHECK (state IN ('planned', 'preview', 'committed', 'skipped', 'failed')), CONSTRAINT "decay_run_times_nonneg" CHECK (scheduled_for_at >= 0 AND created_at >= 0 AND updated_at >= 0)) STRICT;
