...
`tick`, `start_points`, `cap`, `auction_open`, `auction_sealed`, `relative_bid` and `roll` ship today
— see [the worked examples](#the-shipped-strategies) below; the rest of the catalogue follows. This
page explains the design; each strategy's knobs are its `ConfigSchema` in
...
price resolution that could then disagree with it. The four bidding rules refuse in the opposite
direction — `PlanAttendance` and `PlanDecay` return `ErrUnsupported` naming the rule to pair with,
because a spend rule has no tick value and no cadence. That is not a gap — it is what makes a pool
