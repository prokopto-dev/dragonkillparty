...
`tick`, `start_points`, `cap`, `loot_council`, `decay_percent` and `decay_window` ship today — see
...
kind of rule to pair them with. That is not a gap; it is what makes a pool need three rules rather
than one, which is the section below.
