...
|---|---|
| **0** | Grep and lint gates (B3, B4); Actions pinned to SHAs; `ignore-scripts`; licence gate; AGPL firewall grep; `govulncheck`; secret scanning; `FROM scratch` image; migrate-on-boot snapshot and auto-restore. *Landed: the grep gates, SHA pinning, the licence gate, the AGPL firewall, `govulncheck`, `osv-scanner` over both dependency graphs, the enforcing `atlas migrate lint`, and `ignore-scripts` (as a committed `.npmrc` default, not only as a flag at each call site). Outstanding: CI secret scanning, the image and migrate-on-boot.* |
...
| Migration safety | `atlas migrate lint` — destructive, data-dependent and backward-incompatible change analysis over `db/migrations-sqlite/`. **Enforcing** (issue #136, landed advisory-first as #131): `MODE=enforce` is the default and what `make lint-migrations` passes, so a diagnostic fails `make check` and `test / migrations` alike. The waiver is Atlas's own `-- atlas:nolint <analyzer>` beside a `-- dkp:destructive-approved: #<issue>` line, in the migration. Additive to MIG001–003, the fresh-install fingerprint and the populated-upgrade gate, none of which Atlas can express |
