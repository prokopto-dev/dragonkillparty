...
| C3 | **Never `total()` in the ledger.** `sum()` returns an integer for integer inputs and raises on overflow; `total()` always returns a float and never raises. | `total()` over centipoints silently reintroduces float. | `MONEY002` (a text rule) fails `total(`; `MONEY001` fails a float type in `internal/ledger` or `internal/strategy` by reading the parsed Go. Both in `internal/repogate`. |
