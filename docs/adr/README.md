...
| [0024](0024-pure-evaluators-outside-strategy.md) | Pure evaluators outside `internal/strategy` | accepted | A rule evaluator that proposes no batch is its own package and carries its own purity proof; the repo-gate tree list is the upgrade when a third one lands |
