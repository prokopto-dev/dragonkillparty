...
and `docs/api/idempotency-and-concurrency.md` extends it to "a decay, cap or start-points run".

**And there is exactly one run table.** `docs/design/01-domain-model.md` §12.3 defines `decay_run`
and nothing else, with `UNIQUE(pool_id, cadence_period)`.
...
- **Bad, because it diverges from `docs/design/01-domain-model.md` §12.3 as written.** The domain
  model is the schema authority, so this record is what makes the divergence deliberate; §12.3
