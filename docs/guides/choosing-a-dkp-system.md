...
**Status:** nine are implemented — `fixed_price`, `tick`, `start_points`, `cap`, `loot_council`,
`decay_percent`, `decay_window`, `zero_sum` and `attendance_weighted`, whose worked numbers are in
...
([#223](https://github.com/prokopto-dev/dragonkillparty/issues/223)). Numbers for an unshipped rule below are worked from the
