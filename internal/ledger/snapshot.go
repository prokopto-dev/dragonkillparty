...
// UpsertBalanceSnapshot maintains the droppable balance cache ADDITIVELY, keyed on
...
// the sum over ledger_entry, via BalanceAsOfSeq), it is rebuildable, and it is verified nightly. This
// helper must run inside the same store.Tx as the batch write (PR 10) so the cache and the log move
