...
//
// The STRICT DECODE — the null member, the unknown key, the document that is not an object — is
// decodeConfig in common.go, shared with every other strategy in this package. Three rejections were
// found in review of this file and all three were the same defect; sharing the decoder is what stops
// the fourth strategy from having to rediscover them.
func (FixedPrice) config(ctx Ctx) (fixedPriceConfig, error) {
	cfg := defaultFixedPriceConfig()

	if err := decodeConfig(fixedPriceID, ctx.ConfigJSON(), &cfg); err != nil {
...
func (s FixedPrice) PlanAttendance(ctx Ctx, ev AttendanceEvent) (BatchProposal, error) {
	...
	if err := checkDistinctShares(fixedPriceID, attendees); err != nil {
		return BatchProposal{}, err
	}
	...
	for _, a := range attendees {
		if err := checkShare(fixedPriceID, a); err != nil {
			return BatchProposal{}, err
		}
...
// reading THIS strategy's config, because that floor is a fixed_price knob.
func (s FixedPrice) PlanAdjustment(ctx Ctx, ev AdjustmentEvent) (BatchProposal, error) {
	...
	return adjustmentProposal(ctx, fixedPriceID, fixedPriceVersion, cfg.FloorCp, ev)
}
...
//
// A REVERSAL DOES NOT DECLARE NonNegative, AND IT DOES NOT READ THE POOL'S CURRENT CONFIG. Both were
// found in review of the PR that shipped this file — the first would have made real mistakes
// permanently uncorrectable (a floor on a reversal does not prevent a debt, it prevents the
// CORRECTION), and the second would have made every batch in a pool's history unreversible the moment
// its config changed. SumZero still holds and is still declared: a reversal is the exact negation of a
// committed batch, so it can no more mint a point than the original could.
//
// THE BODY IS reversePlan IN common.go, which carries both arguments at length and is what every
// quantity-kind strategy here calls. They live there rather than here because the failure mode of
// forgetting either one is unfixable, and "correct because the next author read this comment" is not
// a property an append-only ledger can rest on.
func (FixedPrice) PlanReversal(ctx Ctx, b LedgerBatch) (BatchProposal, error) {
	return reversePlan(ctx, fixedPriceID, b)
}
...
func (FixedPrice) Spendable(ctx Ctx, acct AccountRef) (core.Centipoints, error) {
	return spendableBalance(ctx, fixedPriceID, acct)
}
...
func (FixedPrice) Priority(ctx Ctx, acct AccountRef) (Priority, error) {
	return priorityBySpendable(ctx, fixedPriceID, acct)
}
...
func (FixedPrice) propose(
	ctx Ctx, kind string, effectiveAt core.Micros, reason string,
	entries []EntryProposal, invariants []Invariant,
) (BatchProposal, error) {
	// No seed: this strategy consumes no randomness, and carrying a seed would assert that replaying
	// the batch from it reproduces the plan — which is true only because the seed is irrelevant.
	// TestFixedPrice_Planners_ConsumeNoRandomness asserts the Rng is never called. proposeZeroSum
	// carries nil for every strategy in this package for the same reason.
	return proposeZeroSum(ctx, fixedPriceID, fixedPriceVersion, kind, effectiveAt, reason,
		entries, invariants)
}
