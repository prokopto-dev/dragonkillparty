...
//
// Three of these are planners OUTSIDE this strategy's slot (ADR-0026 routes attendance and
// adjustments to the earn rule and cadence runs to the over-time rule), and Priority is inside the
// slot and refused on purpose: the council IS the ranking, so a rank computed here would be a number
// the council did not use.
	...
	for _, tc := range []struct {
		...
		{"adjustment", func() error {
			_, err := s.PlanAdjustment(ctx, strategy.AdjustmentEvent{
				Account: strategy.AccountRef{ID: acct(0)}, AmountCp: 100,
			})
