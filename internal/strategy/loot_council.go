...
// PlanAdjustment is unsupported, and this is the first strategy in the tree to say so rather than
// implement it out of habit.
//
// An adjustment routes to the pool's EARN rule (pool.go argues why at length: the floor a member's
// balance lives under between purchases is the one they earn against). A spend rule answering it
// would declare a second, competing floor for a batch no pool would ever route here — `fixed_price`,
// `cap` and `start_points` each implement it because they shipped before that routing existed, not
// because a spend rule owns corrections.
func (LootCouncil) PlanAdjustment(Ctx, AdjustmentEvent) (BatchProposal, error) {
	return BatchProposal{}, Unsupported(lootCouncilID,
		"plan an adjustment: a correction is the pool's earn rule's, which is where its floor lives")
}
