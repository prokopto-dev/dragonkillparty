...
const effectivePermission = `-- name: EffectivePermission :one
    ...
    CAST(EXISTS (
        ...
        WHERE ra.subject_kind = ?1
          ...
          AND (ra.scope_type = 'global'
               OR (ra.scope_type = ?4 AND ra.scope_id = ?5))
    ) AS INTEGER) AS granted
FROM permission p
WHERE p.key = ?6 AND p.orphaned_at IS NULL
`

type EffectivePermissionParams struct {
	SubjectKind   string
	SubjectID     string
	Now           int64
	ScopeType     string
...
func (q *Queries) EffectivePermission(ctx context.Context, arg EffectivePermissionParams) (EffectivePermissionRow, error) {
	row := q.db.QueryRowContext(ctx, effectivePermission,
		arg.SubjectKind,
		arg.SubjectID,
		arg.Now,
		arg.ScopeType,
	...
	var i EffectivePermissionRow
	err := row.Scan(&i.RequiresStepUp, &i.Granted)
	return i, err
}
