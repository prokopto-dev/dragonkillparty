...
func main() {
	...
	baseRef := os.Getenv("DKP_SHIPPED_LOCK_BASE_REF")
	if baseRef == "" {
		baseRef = defaultBaseRef
	}

	os.Exit(run(os.Args[1:], tree{root: root, baseRef: baseRef}, output{stdout: os.Stdout, stderr: os.Stderr}))
}
...
func verify(t tree, complete bool, o output) int {
	...
	case errors.Is(err, fs.ErrNotExist) && complete:
		...
		o.fail("shipped-lock: %s does not exist. A release cannot record which migrations\n", lockFile)
		o.fail("shipped without it. Create it with: go run ./internal/migrate/shippedlock init\n")
	...
	rows, problems, ok := inspect(t, data, complete, o)
	if !ok {
		return exitFail
	}
...
// returns the parsed rows with the problems. ok is false when a check could not run at all — an
...
func inspect(t tree, data []byte, complete bool, o output) (rows []row, problems []string, ok bool) {
...
func parsedIfClean(t tree, data []byte, o output) (rows []row, ok bool) {
	rows, problems, ok := inspect(t, data, false, o)
	if !ok {
		return nil, false
	}
